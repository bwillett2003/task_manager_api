...
Read - Retrieve or read data from database.
Update - Modifies existing data in database.
Delete - Removes data from database.

1. Define MVC.

Model - Responsible for retrieving data from database, manipulate it, update it based on actions performed in the Controller.
View - Displays data to the user.
Controller - Handles user input, processes it, and determines which view to display and a result.
...
Routes and Controller file.

1. What are params? Where do they come from?

Hash object that holds the data sent by the client in a request. Contains key-value pairs where the keys are the names of the parameters and the values are the data sent by the client. Query String parameters, form data, route parameters

1. What is the purpose of a serializer?

Used to control and format the JSON output when renedering data.